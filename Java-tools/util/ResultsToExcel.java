...
//			File output = new File(inputFilePath + "/1/afterData.csv");
			File output = new File(inputFilePath + "/afterDataNEW.csv");
...
//					int i = 94;
				...
				//					1)changes in SATD method		 2)calls OF SATD method	  3)changed calls FROM method	
				// 5,SATD/bug/path,linesAdded,linesRemoved,totalLines,linesAdded,removed,tot,total
				if (afterList != null)
					for (File file : afterList) {
						excel.append(getSatdId(reportPath) + ";");
	
						String filePath = file.toString().substring(len, file.toString().length());
						excel.append(filePath + ";");
	
						int[] satdResult = findLinesChangedFromReport(reportPath, file);
						excel.append(satdResult[0] +";"+ satdResult[1] +";"+ satdResult[2] +";");
	
						int[] callResult = methodInvocation(list[i] + "/report.java", file);
						excel.append(callResult[0] +";"+ callResult[1] +";"+ callResult[2]);
	
						excel.append(";" + callsFromMethod(reportPath, file) + "\n");
					}
				else excel.append(getSatdId(reportPath) + "\n");
