  ...
  #then
    ...
    #that were found between SATD-introd and SATD-fix
        ...
        do
            ...
            bugId="${bug:10:15}"
            echo $bugSha
            mkdir -p "AntBugResults/$id/Between"
            git diff -U1000 $bugSha^ $bugSha -- $filepath > "AntBugResults/$id/Between/${bugId} ${bugSha}_diff.java"
        done < "$output1"  
        echo " "
    #Create folder "After" containing diff files of the Bug Reports 
    #that were found after SATD-fix
        output2=AntBugResults/$id/bugsAfter.txt
        ...
        do
            ...
            bugId="${bug:10:15}"
            echo $bugSha
            mkdir -p AntBugResults/$id/After
            #no file path, in order to have complete diff for the commit
            git diff -U1000 $bugSha^ $bugSha > "AntBugResults/$id/After/${bugId} ${bugSha}_diff.java"
        done < "$output2" 
    ...
    #Print result to file  
        ...
        output=AntBugResults/$id/report.java #java is just to have colors in the Sublime editor :)
        ...
        firstVersionOutput=AntBugResults/$id/first_version.java
